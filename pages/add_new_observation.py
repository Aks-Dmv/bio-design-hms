...
# st.write("This is the Add New Observation page.")
...
def embedObservation(observer, observation, observation_title, observation_date):
    db = Chroma(
    ...
    observation_keys = list(ObservationRecord.__fields__.keys())
    observation_values = [observation_title, observer, observation, observation_date] + [parsed_observation[key] for key in observation_keys]
...
def generateObservationTitle(observation):
...
with col2:
    if st.button("Add Observation to Team Record"):
        st.session_state['observation_title']  = generateObservationTitle(st.session_state['observation'])

        embedObservation(observer, st.session_state['observation'],  st.session_state['observation_title'], 
                         st.session_state['observation_date'])
        st.session_state['result'] = "Generated Title: "+st.session_state['observation_title']+"\n\n"
        st.session_state['result'] += "Observation added to your team's database"
        clear_observation()
        st.experimental_rerun()

# with col3:
#     if st.button("Get Tips from Observation"):
#         st.session_state['result'] = getTipsFromObservation(st.session_state['observation'])
